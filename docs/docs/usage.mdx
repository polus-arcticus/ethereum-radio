...
`safeBlockRangeLimit` applies a safety margin below the detected max, since the true limit may be approximate rather than a hard cutoff. If you already know a safe value for your RPC (your own node, or a provider's documented limit), skip the probe and just set the constant directly instead: `const blockRangeLimit = 2_000n;`.
