...
Also implements `getBlockHash` (via `client.getBlock({blockNumber})`), needed for [`Cursor.checkForReorg`](./core.mdx#checkforreorg).
...
Also implements `getBlockHash` (via `provider.getBlock(blockNumber)`), needed for [`Cursor.checkForReorg`](./core.mdx#checkforreorg).
